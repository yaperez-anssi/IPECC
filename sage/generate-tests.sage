...
#                                                                              #
#                                                                              #
# Parameters: 'NNMINMOD', 'NNMINDECR', 'NNMAXMOD', 'NNMAXDECR' #################
#                                                                              #
...
#   being defined by [nnmin : nnmax].                                          #
#                                                                              #
#   Both 'nnmin' and 'nnmax' are regularly decreased:                          #
#                                                                              #
#     - parameter 'nnmin' is decreased every period of 'NNMINMOD' generated    #
#       curves and by a decrement of 'NNMINDECR'                               #
#                                                                              #
#     - parameter 'nnmax' is decreased every period of 'NNMAXMOD' generated    #
...
remote_board_ended = False

# infinite loop
try:
    while (nbcurv < NBCURV) or (NBCURV == 0):
        if remote_board_ended:
            sys.exit(0)
        if (nn_constant != 0):
            nn = nn_constant
        else:
            if (nbcurv == 0):
                # The first curve is forced to nn = nnmaxabsolute
                nn = nnmaxabsolute
                sys.stderr.write(KWHT + "Generating first curve for nn = " + str(nn) + KNRM + "\n")
            else:
                ...
                if (nbcurv % NNMAXMOD) == NNMAXMOD - 1:
                    prev_max = nnmax
                    nnmax = nnmax - NNMAXDECR;
                    if nnmax < nnmaxmin:
                        nnmax = nnmaxmin
                    new_min_or_max = True
                if (nbcurv % NNMINMOD) == NNMINMOD - 1:
                    prev_min = nnmin
                    nnmin = nnmin - NNMINDECR;
                    if nnmin < nnminmin:
                        nnmin = nnminmin
                    new_min_or_max = True
